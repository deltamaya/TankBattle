...
// 返回值：void
bool ShowMap(const int& level, list<Enemy>& enemy_list, list<Bullet>& bullet_list, Player& player, const Position& base_position, vector<vector<int>>& map, const time_t& play_time);
...
// 方向的枚举，配合DIRECTION使用
enum DIR { NONE = 0, UP = 1, DOWN, LEFT, RIGHT };
// 网格单位的枚举，配合map使用
enum Grid { SPACE = 0, WALL = 1, BASE = 2, CAMP = 3, GRASS = 4 };
// 位置类
class Position {
   public:
    int x_;  // 横坐标
    ...
    bool operator==(const Position& another) const { return x_ == another.x_ && y_ == another.y_; }
    // 位置的方法，传入map，判断地图的这个位置是否可以走
    [[nodiscard]] bool Walkable(const vector<vector<int>>& matrix) const {
        return x_ >= 0 && x_ < matrix.size() && y_ >= 0 && y_ < matrix[0].size() && (matrix[x_][y_] == SPACE || matrix[x_][y_] == GRASS);
    }
};
...
class Bullet {
   public:
    ...
    Bullet(list<Bullet> bullet_list, Position cur_position, DIR direction, bool friendly) : cur_position_(cur_position), direction_(direction), friendly_(friendly){};
};

// 玩家类
class Player {
    static const int MAX_FIRE_DELAY = 1;  // 最大延时，单位为毫秒
   public:
    Position cur_position_;  // 当前位置
    DIR direction_;          // 当前方向
    int hp_;                 // 血量
    int fire_delay_;         // 当前射击延时
    Player(Position cur_position, DIR direction) : cur_position_(cur_position), direction_(direction), hp_(2), fire_delay_(0){};
    Bullet Fire(list<Bullet>& bullet_list) {  // 开火函数，初始化一个子弹，并将射击延时设置为最大延时
        Bullet player_bullet(bullet_list, cur_position_, direction_, true);
        fire_delay_ = MAX_FIRE_DELAY;
        bullet_list.push_back(player_bullet);
...
class Node {
   public:
    ...
    int cur_cost_;
    int predict_cost_;
    Node(const Position& cur_position, Node* pre_node, int cur_cost, int predict_cost) : position_(cur_position), pre_node_(pre_node), cur_cost_(cur_cost), predict_cost_(predict_cost){};
};
struct cmp {
    bool operator()(Node*& a, Node*& b) const { return a->cur_cost_ + a->predict_cost_ > b->cur_cost_ + b->predict_cost_; }
};
...
class Enemy {
    static const int MAX_FIRE_DELAY = 3;
   ...
   public:
    ...
    Enemy(list<Enemy> enemy_list, Position cur_position, DIR direction) : cur_position_(cur_position), direction_(direction), hp_(2), fire_delay_(0), target_(1){};
    void Hit() { --hp_; }
    Bullet Fire(list<Bullet>& bullet_list) {
    ...
    void FireCheck(const vector<vector<int>>& map, list<Bullet>& bullet_list, Player& player, const Position& base_position);
    // 敌人移动，先向初始方向移动一次，然后通过寻路算法更新方向
    void Move(const vector<vector<int>>& map, Player& player, const Position& base_position) {
        cur_position_ = cur_position_ + DIRECTION[direction_];
        DIR dir;
        if (target_)
            dir = AStarAlgorithm(map, cur_position_, player.cur_position_);
        else
            dir = AStarAlgorithm(map, cur_position_, base_position);
        direction_ = dir;
    }
    void Reload() {
        if (fire_delay_ > 0)
